...
const CustomerDashboard = () => {
  ...
  const fetchCustomerTest = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:8082/api/user/test', {
        headers: {
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-content">
        ...
        <div className="test-section">
          <h3>Customer Test Endpoint</h3>
          <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
            Click the button below to test the customer endpoint (requires backend on port 8082)
          </p>
          <button 
            onClick={fetchCustomerTest} 
            className="test-button"
            disabled={loading}
          >
            {loading ? 'Loading...' : 'Test Customer Endpoint'}
          </button>
